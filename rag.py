...
if not st.session_state.public_url:
    ...
    chunk_overlap = st.sidebar.slider("Chunk Overlap", 0, 100, 30)
                        ...
                        try:
                            ...
                            if 'chunks' in result:
                                ...
                                for rank, chunk in enumerate(result["chunks"], 1):
                                    ...
                                    with st.expander(f"Rank #{rank} (Chunk {chunk_index})", expanded=False):
                                        st.markdown(f"**Source**: {chunk['source']}")
                                        st.markdown(f"**Score**: {chunk['score']}")
                                        st.markdown(f"**Reference**: {chunk['reference']}")
